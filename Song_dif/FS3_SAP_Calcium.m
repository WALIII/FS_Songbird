function [DAT] = FS3_SAP_Calcium(A)
...
startT = 0.25;
stopT = 0.750;
A.mic_data = A.analogIO_dat;
 
% compute SAP scores and smooth Mic data
for i = 1:size(A.align_detrended,2)
[F{counter},P{counter}]=zftftb_sap_score(A.mic_data{i}(startT*FS:end-stopT*FS),FS);
counter = counter+1;
end

% Compare all to all:
counter = 1;
for c = 1:size(A.align_detrended,2)
for i = 1:size(A.align_detrended,2)

% Mic data
MICDATA = A.mic_data{i}(FS*startT:end-stopT*FS); % Change on each itteration
C_MICDATA = A.mic_data{c}(FS*startT:end-stopT*FS); % Change on each larger loop. to compare

% CA imaging data
IMDAT = (A.align_detrended{i}(round(startT*30):end-round(stopT*30),:));
C_IMDAT = (A.align_detrended{c}(round(startT*30):end-round(stopT*30),:));
